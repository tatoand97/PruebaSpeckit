...
if ($LocalDistributionRoot) {
    ...
    foreach ($name in @('presets.json', 'workflows.json', 'extensions.json', 'bundles.json')) {
        ...
        switch ($name) {
            ...
            'extensions.json' {
                $document.extensions.'dotnet-sdd-guard'.download_url = "$localBase/artifacts/dotnet-sdd-guard-1.0.2.zip"
            }
            'bundles.json' {
                $document.bundles.'dotnet-sdd'.download_url = "$localBase/artifacts/dotnet-sdd-bundle-1.0.2.zip"
            }
        }
...
try {
    ...
    if ($extensionManifest -notmatch '(?m)^\s*version:\s*"?1\.0\.2"?\s*$') { throw 'Installed extension version is not 1.0.2.' }
    ...
    if ($record.Count -ne 1 -or $record[0].version -ne '1.0.2') {
        throw 'Bundle provenance for dotnet-sdd 1.0.2 was not recorded.'
    }
