...
- [ ] T012 Verify schema initialization, connection configuration and SQLite invariants in `tests/Orders.Api.Tests/PersistenceTests.cs`: schema/file creation and reuse, exact columns/constraints, `STRICT`, WAL, foreign keys, `synchronous=FULL`, `busy_timeout=500`, `quick_check`, empty `foreign_key_check` and fail-fast startup for incompatible/corrupt/unwritable storage; do not require business operations that do not exist yet (Plan §Persistence/Failure boundaries; FR-014)
